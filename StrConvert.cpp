...
// CString을 std::string으로 변환하는 함수
std::string CStringToString(const CString& cstr)
{
    // CString의 내용을 std::string으로 변환
    CT2CA pszConvertedAnsiString(cstr); // TCHAR to ANSI string (CString to const char*)
    std::string strStd(pszConvertedAnsiString); // const char* to std::string
    return strStd;
}

// std::string을 CString으로 변환하는 함수
...
// String table의 문자열을 CString으로 변환하는 함수
CString StringTableToCString(UINT strID)
{
	CString cstr;
	cstr.LoadString(strID);
	return cstr;
}
